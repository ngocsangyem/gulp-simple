...
module.exports = function(component, type, page, deps, extnames, imports) {
	const modules = (deps[component] && deps[component].modules) || [];
	const async = /@(async|defer)/gi;
	const node = page.nodes && page.nodes[component];
	const data = {
		name: component,
	...
	// Check all dependencies modules

	modules.forEach(module => {
		if (!module || module.constructor !== Object) {
			return console.log(
				colors.red("Dependency module must be a object!")
			);
		}
